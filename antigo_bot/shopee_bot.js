...
const CONFIG = {
    ...
    groupId: process.env.SHOPEE_GROUP_ID,
    intervaloMinutos: 5
};
...
let sentIds = [];
try {
    if (fs.existsSync('sent_ids.json')) {
        const data = fs.readFileSync('sent_ids.json', 'utf8');
        sentIds = JSON.parse(data);
    }
} catch (err) {
    console.error('Erro ao ler sent_ids.json:', err);
}
...
async function executarCiclo() {
    try {
        ...
        // Salva arquivo
        try {
            fs.writeFileSync('sent_ids.json', JSON.stringify(sentIds, null, 2));
        } catch (err) {
            console.error('Erro ao salvar sent_ids.json:', err);
        }
